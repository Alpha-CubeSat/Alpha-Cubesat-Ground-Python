...
img_fragment_downlink_info = {'capture_serial': 0, 'latest_fragment': 0, 'missing_fragments': [],
...
def save_fragment(imei: int, capture_sn: int, fragment_number: int, fragment_data: str):
    ...
    img_fragment_downlink_info['capture_serial'] = capture_sn
...
def try_save_capture(imei: int, capture_sn: int, total_fragments: int):
    ...
    equals the total # of fragments, the completed capture is saved to the directory 'img' as a
    ...
    if not exists(f'{cfg.capture_root_dir}/{imei}/img'):
        os.makedirs(f'{cfg.capture_root_dir}/{imei}/img')

    with open(f'{cfg.capture_root_dir}/{imei}/img/{capture_sn}.jpg', 'wb') as capture_file:
...
def replace_capture_fragment(imei: str, capture_file_name: str, fragment_number: int, fragment_data: str):
    ...
    with open(f'{cfg.capture_root_dir}/{imei}/img/{capture_file_name}.jpg', 'rb') as capture_file:
        current_img = capture_file.read().hex()
    ...
    with open(f'{cfg.capture_root_dir}/{imei}/img/{capture_file_name}.jpg', 'wb') as capture_file:
        capture_file.write(bytearray.fromhex(new_img))
