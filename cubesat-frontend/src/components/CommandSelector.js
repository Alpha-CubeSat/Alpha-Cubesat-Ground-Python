...
export default function CommandSelector() {
  ...
  const handleFirstArgSelection = (arg1) => () => {
    ...
    setDesc("Select a command");
    setDesc(
      selectArguments[selectedOpCode].desc[
        selectArguments[selectedOpCode].arg.indexOf(arg1)
      ]
    );
          ...
          <Dropdown>
            ...
            <Dropdown.Menu>
              {opcodeList.map((option, index) => (
                <Dropdown.Item
                  key={index}
                  onClick={handleOpCodeSelection(option)}
                >
                  {option}
                </Dropdown.Item>
              ))}
          ...
          <Dropdown>
            ...
            <Dropdown.Menu>
              {firstArgList.map((option, index) => (
                <Dropdown.Item
                  key={index}
                  onClick={handleFirstArgSelection(option)}
                >
                  {option}
                </Dropdown.Item>
              ))}
