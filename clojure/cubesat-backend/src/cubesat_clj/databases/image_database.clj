...
(defn- generate-missing-fragments 
  ...
  (let [max-frag (apply max frag-list)]
    ...
    (swap! img-display-info assoc :highest-fragment max-frag)
    (loop [x 0]
      (when (< x max-frag)
        (if (some #(= x %) frag-list)
          ()
...
(defn try-save-image
  ...
  (let [db-root-dir (get-root-path)
        ...
        string-total (if (= -1 total-fragment-number) (str num-fragments "/?") (str num-fragments "/" total-fragment-number))
