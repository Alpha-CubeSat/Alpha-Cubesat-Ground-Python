...
(defn cycle-db-index
  "Returns the configured ElasticSearch index base name for imu cycle data."
  [config]
  (get-in config [:telemetry :elasticsearch-indices :cycle]))

(defn deploy-db-index
  "Returns the configured ElasticSearch index base name for deployment data."
  [config]
  (get-in config [:telemetry :elasticsearch-indices :deploy]))
...
(defn cycle-db-index
  "Returns the configured ElasticSearch index base name for IMU downlink cycle data."
  [config]
  (get-in config [:telemetry :elasticsearch-indices :cycle]))
